...
const TRACE_PREFIX = 'MIOT_TRACE';

function traceEnabled() {
  return process.env.MIOT_TRACE === '1';
}

function traceEscape(val) {
...
class MiioProtocol extends EventEmitter {
  ...
  _trace(address, step, pairs) {
    if (!traceEnabled()) return;
    const parts = [TRACE_PREFIX, `addr=${traceEscape(address)}`, `step=${traceEscape(step)}`];
    for (const [k, v] of pairs) {
      parts.push(`${k}=${traceEscape(v)}`);
    }
  ...
  _decryptMessage(address, msg) {
    ...
    if (encrypted.length === 0) {
      this._trace(address, 'handshake_recv', [
        ['len', msg.length],
        ['magic_hex', msg.slice(0, 2).toString('hex')],
        ['length', msg.readUInt16BE(2)],
        ['unknown', msg.readUInt32BE(4)],
        ['did', deviceId],
        ['did_hex', deviceId.toString(16).padStart(8, '0')],
        ['stamp', stamp],
        ['stamp_hex', stamp.toString(16).padStart(8, '0')],
        ['checksum_hex', checksum.toString('hex')],
        ['header_hex', msg.slice(0, 32)],
      ]);

      this._trace(address, 'handshake_values', [
        ['did', deviceId],
        ['did_hex', deviceId.toString(16).padStart(8, '0')],
        ['base_stamp', stamp],
        ['base_stamp_hex', stamp.toString(16).padStart(8, '0')],
        ['base_time_ms', device._serverStampTime],
      ]);
  ...
  _encryptMessage(address, data) {
    ...
    this._trace(address, 'token_derive', [
      ['token', device._token],
      ['token_key', device._tokenKey],
      ['token_iv', device._tokenIV],
    ]);

    this._trace(address, 'plaintext_json', [
    ...
    this._trace(address, 'encrypt_aes128_cbc', [
    ...
    const computedStamp = device._serverStamp + secondsPassed;
    header.writeUInt32BE(computedStamp, 12);

    this._trace(address, 'stamp_compute', [
      ['base_stamp', device._serverStamp],
      ['base_stamp_hex', Number(device._serverStamp).toString(16).padStart(8, '0')],
      ['base_time_ms', device._serverStampTime],
      ['now_ms', Date.now()],
      ['seconds_passed', secondsPassed],
      ['stamp', computedStamp],
      ['stamp_hex', Number(computedStamp).toString(16).padStart(8, '0')],
    ]);

    // Device ID
    header.writeUInt32BE(Number(device.did), 8);

    this._trace(address, 'header_before_checksum', [
      ['magic_hex', '2131'],
      ['length', 32 + encrypted.length],
      ['unknown', 0],
      ['did', Number(device.did)],
      ['did_hex', Number(device.did).toString(16).padStart(8, '0')],
      ['stamp', computedStamp],
      ['stamp_hex', Number(computedStamp).toString(16).padStart(8, '0')],
      ['header16_hex', header.slice(0, 16)],
    ]);
    ...
    this._trace(address, 'checksum_md5', [
      ['md5', digest],
      ['header32_hex', header],
    ]);

    const pkt = Buffer.concat([header, encrypted]);
    this._trace(address, 'packet', [
      ['len', pkt.length],
      ['hex', pkt],
    ]);
  ...
  _handshake(address) {
    ...
    this._trace(address, 'handshake_send', [
      ['port', PORT],
      ['len', msg.length],
      ['hex', msg],
    ]);
    this.logger.info(`(Protocol) Handshake -> ${address}:${PORT} (${msg.length} bytes)`);
    this.logger.info(`(Protocol) Handshake HEX -> ${msg.toString('hex')}`);
    // this.logger.info(
    //   '(Protocol) Handshake REPLAY (python) ->\n'
    //   + 'import socket\n'
    //   + `msg = bytes.fromhex("${msg.toString('hex')}")\n`
  ...
  _send(address, json) {
    ...
    this.logger.info(`(Protocol) Send -> ${address}:${PORT} id=${id} method=${method} (${msg.length} bytes)`);
    this.logger.info(`(Protocol) Send HEX -> ${msg.toString('hex')}`);
    // this.logger.info(
    //   '(Protocol) Send REPLAY (python) ->\n'
    //   + 'import socket\n'
    //   + `msg = bytes.fromhex("${msg.toString('hex')}")\n`
    //   + 's = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)\n'
    //   + `s.sendto(msg, ("${address}", ${PORT}))\n`
  ...
  async send(address, method, params = [], options = {}) {
    this.logger.info(`(Protocol) Call ${address}: ${method} - ${JSON.stringify(params)} - ${JSON.stringify(options)}`);
    const request = {
      method,
      params,
    };
